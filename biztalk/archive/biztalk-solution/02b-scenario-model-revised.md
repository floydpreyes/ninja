...
## Out-of-bucket workstreams (add on top if discovery surfaces them)

These three BizTalk features have **no 1:1 AIS equivalent** and are not
absorbed in the bucket model above. Discovery (questions 13–15 in
[07-questionnaire.md](07-questionnaire.md)) confirms presence/absence. If
present, treat each as a separate engineering line item.

| Feature | What it is | AIS replacement | Engineering effort to add |
|---|---|---|---|
| **EDI** (X12, EDIFACT, HL7) | BizTalk's B2B engine — trading-partner agreements, envelope/batch processing, 997/CONTRL acks | Logic Apps **Enterprise Integration Pack** (B2B agreements + AS2/X12/EDIFACT connectors), or third-party (Cleo, OpenText) | **40–80 d** for EIP setup + first agreement; **+10–15 d** per additional partner |
| **Custom adapter** | C# adapter MW developers wrote to talk to a system the built-in adapters don't support (mainframe, vendor SDK, proprietary protocol) | Depends on protocol — see breakdown below | **5–50 d** per adapter (see table) |
| **Large BAM footprint** | Business Activity Monitoring — tracking profiles tied to business rules, BAM portal / Excel pivots over the BAM data warehouse | Application Insights + Log Analytics + **Azure Workbooks** for engineering telemetry; **Power BI / Fabric** over Log Analytics for business reports | **20–60 d** depending on number of activities and stakeholder validation cycles |

### Custom adapter sub-breakdown

| Custom adapter type | First-of-pattern engineering days | Notes |
|---|---|---|
| Native AIS connector exists (SAP, MQ, EIP) — config only | 5–8 d | Enterprise connector tier — adds ongoing licence cost |
| Logic App custom connector (declarative HTTP wrapper) | 10–15 d | Suitable for proprietary REST/SOAP with auth quirks |
| Function wrapping a vendor SDK | 15–25 d | When SDK only available as .NET library |
| Container App with bespoke protocol listener | 25–40 d | Custom TCP, framing, lifecycle management |
| Mainframe / Host Integration Server install + config | 30–50 d | Separate workstream; engage procurement early |

Subsequent integrations that **reuse** the same custom adapter follow normal
subsequent-day rates from the bucket model.

### How this affects scenario choice

| If discovery finds… | Action |
|---|---|
| 0 of these features | Estimates stand. Use Scenario A. |
| 1 small instance (≤ 15 d) | Absorbed by Scenario B's discovery surprise reserve |
| 1 medium instance (15–40 d) | Add to Scenario A as separate line; recommend Scenario B baseline |
| 1 large instance (> 40 d) | Add as separate line; recommend Scenario C baseline |
| 2 or more, or any EDI workstream | **Scope change** — separate engineering workstream with dedicated engineer; re-baseline programme |

### Other considerations

- **Licensing:** Logic Apps Enterprise connectors (SAP, MQ, IBM DB2, Oracle EBS, EIP) require the Enterprise tier — ongoing cost, not just engineering effort.
- **Hybrid runtime:** If the legacy system is on-prem and unreachable from Azure, the replacement may need to run on the **Logic Apps Standard hybrid runtime** — extends Wave 1 perimeter work.
- **Skill demand:** Custom adapters bump the .NET line item in the skills demand chart in [06-effort-estimation.md](06-effort-estimation.md).
- **BAM stakeholder cost:** BAM rebuilds are stakeholder-heavy because business users must validate that new dashboards match the old reports — PM owns this elapsed time.
